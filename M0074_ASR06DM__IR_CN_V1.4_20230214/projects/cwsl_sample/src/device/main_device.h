...
typedef enum
{
    ...
    MAIN_ASR_RESTART = MAIN_ASR_START + 300,                        //恢复出厂设置
...
typedef struct
{
    ...
    int iWakeUpIndex;
    bool Encrypt_Pass;/* 密码检测结果   true:正确   false：错误 */
    bool InvalidMode;/* 失效机制 true:识别失效   false：正常识别 */
    eDeviceModeCtl eTvStbSwitch;
    int restart_flag;       //恢复出场设置标准位
}stMainStatus;
