...
stMainStatus* getstBabyStaInfo(void)
{
    return &stBabySta;
}
...
static int main_device_playid(stDevice* pdevice, stAsrInfo* pAsrInfo)
{
    ...
    switch(pAsrInfo->semantic_id)
    {
        ...
        case MAIN_ASR_CANCEL_CLEAN_CODE:
            ...
            break;
        case MAIN_ASR_RESTART:                                      //恢复出厂设置
            {
                /* 清除各红外码 */
                // get_air_device()->CleanIrData;
                // get_fan_device()->CleanIrData;
                // get_tv_device()->CleanIrData;
                // get_light_device()->CleanIrData;
                air_cleanIrCode();
                fan_cleanIrCode();
                tv_cleanIrCode();
                light_cleanIrCode();
                ...
                /* 清除自学习数据 */
                stBabySta.restart_flag = 1;
                cwsl_app_reset();                   //先自学习复位，防止状态异常
                qlife_cwsl_delete_word();           //删除自学习的唤醒词
