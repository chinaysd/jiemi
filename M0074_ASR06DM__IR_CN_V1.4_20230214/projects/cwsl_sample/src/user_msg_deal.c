...
//轻生活密码检测
xTimerHandle xTimerPassword = NULL;
...
void qilfe_check_password_deal(void)
{
    xTimerPassword = xTimerCreate("xTimerPassword", (pdMS_TO_TICKS(1000)), pdTRUE, (void *)0, CheckPasswordTimerCallback);
    if(!xTimerPassword)
    {
        mprintf("xTimerPassword fail!\n");
    }    
    else
    {
        xTimerStart(xTimerPassword, 0);
    }
}

static uint32_t Time_cnt = 0;
void CheckPasswordTimerCallback(TimerHandle_t xTimer)
{
    if(getstBabyStaInfo()->Encrypt_Pass == false)                   //密码错误
    {
        Time_cnt++;
    }

    if((Time_cnt >= 60*60) && (Time_cnt <= 65*60))                  //05~65分钟 失效
    {
        getstBabyStaInfo()->InvalidMode = true;
    }
    else if(Time_cnt > 65*60)
    {
        Time_cnt = 0;
        getstBabyStaInfo()->InvalidMode = false;
    }
    else
    {
        /* 正常使用期间 */
